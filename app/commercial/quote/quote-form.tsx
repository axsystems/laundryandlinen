  ...
  useEffect(() => {
    if (industryParam) {
      const industryMap: Record<string, string> = {
        ...
        apartments: "Apartment Complex / Multi-Family",
        gyms: "Gym / Fitness Center",
        "salons-spas": "Spa / Salon",
        healthcare: "Healthcare / Medical",
        "event-venues": "Other",
      };
      const mapped = industryMap[industryParam];
      if (mapped) {
        setFormData((prev) => ({ ...prev, businessType: mapped }));
      }
    }
  }, [industryParam]);
    ...
    <div className="pt-24 md:pt-32 pb-16">
      <div className="container-custom">
        <div className="max-w-3xl mx-auto">
          ...
          {(industryParam || cityParam) && (
            <div className="mb-6 p-4 rounded-xl bg-primary/5 border border-primary/20">
              <p className="text-sm text-foreground">
                <span className="font-medium">Requesting a quote</span>
                {industryParam && <span> for <span className="text-primary font-medium">{industryParam.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}</span> services</span>}
                {cityParam && <span> in <span className="text-primary font-medium">{cityParam.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}, AZ</span></span>}
              </p>
            </div>
          )}
